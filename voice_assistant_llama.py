...
model_path = "/home/roshni/Project/lamma/src/llama-3.2-3B-Instruct-bnb-4bit-finetuned"
...
def generate_llama_completion(history):
    system_message = (
        "System: You are the official voice assistant of the University of Massachusetts Dartmouth, College of Engineering. "
        "Your role and responsibility is to attend to students and assist them. Be a helpful and informative assistant.\n"
    )
...
def response(state: AppState, audio: tuple):
    ...
    if transcription:
        ...
        assistant_message = generate_llama_completion(state.conversation_history)
...
with gr.Blocks(theme=theme, js=js) as demo:
    gr.Markdown("# 🎤 Voice Assistant (Llama Local Model)")
